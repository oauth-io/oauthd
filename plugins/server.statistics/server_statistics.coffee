...
exports.setup = (callback) ->
	...
	@server.get @config.base + '/api/apps/:key/stats', @auth.needed, (req, res, next) =>
		req.params.target = 'co:a:' + req.params.key
		sendStats req.params, @server.send(res, next)

	# get statistics for a keyset
	@server.get @config.base + '/api/apps/:key/keysets/:provider/stats', @auth.needed, (req, res, next) =>
		req.params.target = 'co:a:' + req.params.key + ':p:' + req.params.provider
		sendStats req.params, @server.send(res, next)

	# get statistics for a provider
	@server.get @config.base + '/api/providers/:provider/stats', @auth.needed, (req, res, next) =>
